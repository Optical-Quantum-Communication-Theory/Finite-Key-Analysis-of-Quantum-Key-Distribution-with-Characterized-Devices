...
filename = 'bb84v3alg2exploret.mat';
...
t_range = [0, 0.005,0.01,0.015,0.02];
n_t = length(t_range);
...
maxKeyList = zeros(n_t,length(n_range));
minTFcell = cell(n_t,length(n_range));

exitList=  cell(n_t,length(n_range));
upperList = cell(n_t,length(n_range));
keyList =cell(n_t,length(n_range));

for t_index = 1:n_t
    acceptance_t = t_range(t_index);
    ...
    parfor i = 1:length(n_range)
        ...
        minTFcell{t_index, i} = tmpminCell;
        exitList{t_index, i} =  tmpexitList;
        upperList{t_index, i} = tmpupList;
        keyList{t_index, i}  = tmpkeyList;
        maxKeyList(t_index, i) = max(max(tmpkeyList),0);
    end
end
toc
save(filename);
